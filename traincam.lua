...
local zoom_map = { 0.25, 0.325, 0.4, 0.5, 0.6, 0.75, 1, 1.25, 1.5, 2, 2.5, 3, 3.5, 4 }
...
function traincam.apply_settings(player, id)
    ...
    if cam_state and cam_state.settings_size_input and cam_state.settings_size_input.valid then
        ...
        if new_size then
            if new_size < 150 then
                new_size = 150
            end
            if new_size > 5000 then
                new_size = 5000
            end
...
function traincam.apply_size(player, id)
    ...
    if cam_state and cam_state.main and cam_state.main.valid then
        ...
        if input and input.valid then
            ...
            if new_size then
                if new_size < 250 then
                    new_size = 250
                end
                if new_size > 2000 then
                    new_size = 2000
                end
...
function traincam.on_click(player, element)
    ...
    if not tags or not tags.traincam_id then
        return
    end
...
function traincam.tick()
    if not storage.traincam_players then
        return
    end
    ...
    for player_index, data in pairs(storage.traincam_players) do
        ...
        if player then
            -- On récupère la vitesse définie par le joueur dans ses paramètres
                ...
                if target and target.valid then
                    ...
                    if should_update_titles and player and cam_state.title_label and cam_state.title_label.valid then
                        local surface_trains =
                            #game.train_manager.get_trains({surface = target.surface, force = player.force})
                        if surface_trains <= 1 then
                    ...
                    -- Le calcul utilise maintenant le paramètre "lock_f" du joueur
                    ...
                    if cam_state.camera and cam_state.camera.valid then
                        ...
                        cam_state.camera.zoom = zoom_map[cam_state.zoom or 7]
